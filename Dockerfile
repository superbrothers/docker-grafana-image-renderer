# based on https://github.com/grafana/grafana-image-renderer/blob/master/Dockerfile
...
ARG IMAGE_RENDERER_VERSION

RUN set -x && \
    apk add --no-cache git

RUN set -x && \
    rm -rf /usr/source/app && \
    git clone --depth 1 --branch "$IMAGE_RENDERER_VERSION" https://github.com/grafana/grafana-image-renderer.git /usr/src/app
